...
template <typename U> struct FilterVector {
  std::vector<U> data;
  ...
  explicit FilterVector(size_t size, const U &initial_value = U{})
  ...
  FilterVector operator+(const FilterVector &other) const {
    if (data.size() != other.data.size()) {
      throw std::runtime_error(fmt::format(
          "Vectors must be of the same size for addition ({} vs {}).",
...
template <typename T> struct FilterTraits {
  ...
  static void validate_input(const T &data_in,
...
template <> struct FilterTraits<sensor_msgs::msg::JointState> {
  using DataType = sensor_msgs::msg::JointState;
  using StorageType = Eigen::MatrixXd;

  static void initialize(StorageType &storage) {
    storage = StorageType::Constant(2, storage.cols(),
                                    std::numeric_limits<double>::quiet_NaN());
  }

  static bool is_nan(const StorageType &storage) {
    return !((storage.array() == storage.array()).all());
  }

  static bool is_finite(const DataType &data) {
    for (const auto &val : data.position)
      if (!std::isfinite(val))
        return false;
    for (const auto &val : data.velocity)
      if (!std::isfinite(val))
        return false;
    return true;
  }
  ...
  static bool is_empty(const StorageType &storage) {
    return storage.cols() == 0;
  }

  static void assign(StorageType &storage, const DataType &data_in) {
    const size_t n = data_in.position.size();

    if (!data_in.velocity.empty() && data_in.velocity.size() != n) {
      throw std::runtime_error(
          "Position and velocity vectors must have the same size.");
    }

    storage.resize(2, n);
    for (size_t i = 0; i < n; ++i) {
      storage(0, i) = data_in.position[i];
      storage(1, i) = data_in.velocity.empty()
                          ? std::numeric_limits<double>::quiet_NaN()
                          : data_in.velocity[i];
    }
  }

  static void assign(DataType &data_out, const StorageType &storage) {
    size_t n = storage.cols();
    data_out.position.resize(n);
    data_out.velocity.resize(n);
    for (size_t i = 0; i < n; ++i) {
      data_out.position[i] = storage(0, i);
      data_out.velocity[i] = storage(1, i);
    }
  }
  ...
  static void validate_input(const DataType &data_in,
                             const StorageType &filtered_value,
                             DataType &data_out) {
    (void)filtered_value;

    if (!data_out.name.empty() && data_out.name != data_in.name) {
      throw std::runtime_error("Joint names changed between filter updates");
    }
  }
...
template <> struct FilterTraits<geometry_msgs::msg::WrenchStamped> {
  using StorageType = Eigen::Matrix<double, 6, 1>;
  using DataType = geometry_msgs::msg::WrenchStamped;

  static void initialize(StorageType &storage) {
    // Evocation of the default constructor through
  ...
  static bool is_finite(const DataType &data) {
    return std::isfinite(data.wrench.force.x) &&
           std::isfinite(data.wrench.force.y) &&
           std::isfinite(data.wrench.force.z) &&
           std::isfinite(data.wrench.torque.x) &&
           std::isfinite(data.wrench.torque.y) &&
           std::isfinite(data.wrench.torque.z);
  }
  ...
  static void validate_input(const DataType &data_in,
    ...
    if (!data_out.header.frame_id.empty() &&
      ...
      throw std::runtime_error(
          "Frame ID changed between filter updates! Out: " +
          data_out.header.frame_id + ", In: " + data_in.header.frame_id);
    }
  }
...
template <typename U> struct FilterTraits<std::vector<U>> {
  ...
  static bool is_finite(const StorageType &storage) {
    return std::all_of(storage.data.begin(), storage.data.end(),
                       [](U val) { return std::isfinite(val); });
  }
  ...
  static void assign(StorageType &storage, const DataType &data_in) {
    storage.data = data_in;
  }

  static void assign(DataType &storage, const StorageType &data_in) {
    storage = data_in.data;
  }
  ...
  static void validate_input(const DataType &data_in,
    ...
    if (!data_out.empty() && data_out.size() != data_in.size()) {
      throw std::runtime_error(fmt::format(
          "Input and output vectors must be the same size, {} vs {}.",
