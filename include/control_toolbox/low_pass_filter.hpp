...
public:
  ...
  LowPassFilter(double sampling_frequency, double damping_frequency,
  ...
  /*!
   * \brief Internal computation of the feedforward and feedbackward
   * coefficients according to the LowPassFilter parameters.
   */
  void set_params(double sampling_frequency, double damping_frequency,
                  double damping_intensity) {
...
template <typename T> LowPassFilter<T>::LowPassFilter() : a1_(1.0), b1_(0.0) {}
...
bool LowPassFilter<T>::update(const T &data_in, T &data_out) {
  ...
  // If this is the first call to update initialize the filter at the current
  // state so that we dont apply an impulse to the data.
